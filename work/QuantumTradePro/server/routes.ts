...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // NEW: Intraday trading routes
  app.get('/api/stock/:symbol/intraday', async (req, res) => {
    try {
      const symbol = stockSymbolSchema.parse(req.params.symbol.toUpperCase());
      
      // Get current stock data
      const stockPrice = await stockService.getCurrentPrice(symbol);
      if (!stockPrice) {
      ...
      // Generate intraday recommendation
      const recommendation = await aiService.generateIntradayRecommendation(
        symbol, 
        stockPrice, 
        indicators, 
        news
      ...
      console.error('Error generating intraday recommendation:', error);
      res.status(400).json({ error: 'Invalid stock symbol' });
    }
  });

  app.get('/api/market/morning-recommendations', async (req, res) => {
    try {
      // Get morning recommendations for popular stocks
      const popularStocks = ['RELIANCE', 'TCS', 'INFY', 'HDFCBANK', 'ICICIBANK', 'ADANIGREEN'];
      const recommendations = await aiService.generateMorningRecommendations(popularStocks);
      ...
      console.error('Error generating morning recommendations:', error);
      res.status(500).json({ error: 'Failed to generate morning recommendations' });
    }
  });

  app.get('/api/market/intraday-scanner', async (req, res) => {
    try {
      const symbols = req.query.symbols ? String(req.query.symbols).split(',') : ['RELIANCE', 'TCS', 'INFY'];
      ...
      for (const symbol of symbols) {
        try {
          ...
          if (stockPrice && indicators) {
            ...
            intradaySignals.push({
              symbol,
              price: stockPrice.price,
              change: stockPrice.changePercent,
              volume: stockPrice.volume,
              signal: recommendation.recommendation,
              confidence: recommendation.confidence,
              reason: recommendation.reasoning[0] || 'Analysis completed'
            });
          }
        } catch (error) {
          console.log(`Failed to analyze ${symbol}:`, error);
        }
      }
      ...
      res.json({
        ...
        signals: intradaySignals.sort((a, b) => b.confidence - a.confidence)
      });
    } catch (error) {
      console.error('Error in intraday scanner:', error);
