...
export class StockService {
  private indianAPIUrl = 'https://indian-stock-api.onrender.com'; // Free Indian stock API
  private nseBseAPIUrl = 'https://api.indianapi.in'; // Indian API service
  ...
  async getCurrentPrice(symbol: string): Promise<StockPrice | null> {
    try {
      ...
      // Try backup Indian API service
      const backupData = await this.fetchFromIndianAPIService(symbol);
      if (backupData) {
        return backupData;
      }

      // If all real APIs fail, return null to indicate no data available
      console.error(`All real APIs failed for ${symbol}. No mock data returned.`);
      return null;
    } catch (error) {
  ...
  private async fetchFromIndianAPIService(symbol: string): Promise<StockPrice | null> {
    try {
      // Use IndianAPI.in service for NSE stock data
      const response = await fetch(`${this.nseBseAPIUrl}/nse/stock/${symbol}`);
      
      if (!response.ok) {
        throw new Error(`Indian API Service failed: ${response.status}`);
      }
      ...
      if (data && data.success && data.data) {
        ...
        return {
          symbol,
          price: parseFloat(stock.lastPrice) || parseFloat(stock.price) || 0,
          change: parseFloat(stock.change) || 0,
          changePercent: parseFloat(stock.pChange) || parseFloat(stock.percentChange) || 0,
          volume: parseInt(stock.volume) || parseInt(stock.totalTradedVolume) || 0,
          marketCap: parseFloat(stock.marketCap) || 0,
          high52w: parseFloat(stock.high52w) || parseFloat(stock.yearHigh) || 0,
          low52w: parseFloat(stock.low52w) || parseFloat(stock.yearLow) || 0,
          previousClose: parseFloat(stock.previousClose) || parseFloat(stock.previousPrice) || 0,
          openPrice: parseFloat(stock.open) || parseFloat(stock.openPrice) || 0,
          lastUpdated: new Date()
        };
      }
    } catch (error) {
      console.log(`Indian API Service failed for ${symbol}:`, error instanceof Error ? error.message : error);
    }
    return null;
  }
  ...
  async getHistoricalPrices(symbol: string, timeframe: string): Promise<HistoricalPrice[]> {
    try {
      ...
      // If all real APIs fail, return empty array
      console.log(`All real historical data APIs failed for ${symbol}. No mock data returned.`);
      return [];
    } catch (error) {
  ...
  private async fetchIndianHistoricalData(symbol: string, timeframe: string): Promise<HistoricalPrice[]> {
    try {
      // Try FREE GitHub Indian Stock Market API first
      const githubData = await this.fetchFromGitHubIndianAPI(symbol, timeframe);
      if (githubData && githubData.length > 0) {
        console.log(`Fetched ${githubData.length} historical data points from GitHub Indian API for ${symbol}`);
        return githubData;
      }

      // Try alternative Yahoo Finance historical approach  
      const altYahooData = await this.fetchAlternativeYahooData(symbol, timeframe);
      if (altYahooData && altYahooData.length > 0) {
        console.log(`Fetched ${altYahooData.length} historical data points from alternative Yahoo for ${symbol}`);
        return altYahooData;
      }

      // Try NSE Direct API (free)
      const nseData = await this.fetchNSEDirectData(symbol, timeframe);
      if (nseData && nseData.length > 0) {
        console.log(`Fetched ${nseData.length} historical data points from NSE Direct for ${symbol}`);
        return nseData;
      }

    } catch (error) {
      console.log(`All Indian historical APIs failed for ${symbol}:`, error);
    }
    return [];
  }

  private async fetchFromGitHubIndianAPI(symbol: string, timeframe: string): Promise<HistoricalPrice[]> {
    try {
      // Use the free GitHub stock-market-india API
      const response = await fetch(`https://stock-market-india.onrender.com/nse/get_historical_data?symbol=${symbol}&period=${timeframe}`);
      
      if (!response.ok) {
        throw new Error(`GitHub Indian API failed: ${response.status}`);
      }
      ...
      console.log(`GitHub Indian API failed for ${symbol}:`, error);
    }
    return [];
  }

  private async fetchAlternativeYahooData(symbol: string, timeframe: string): Promise<HistoricalPrice[]> {
    try {
      const yahooSymbol = `${symbol}.NS`;
      const days = this.getTimeframeDays(timeframe);
      
      // Use alternative Yahoo Finance API endpoint
      const period1 = Math.floor((Date.now() - days * 24 * 60 * 60 * 1000) / 1000);
      const period2 = Math.floor(Date.now() / 1000);
      
      const url = `https://query2.finance.yahoo.com/v8/finance/chart/${yahooSymbol}?period1=${period1}&period2=${period2}&interval=1d`;
      
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error(`Alternative Yahoo API failed: ${response.status}`);
      }
      ...
      console.log(`Alternative Yahoo API failed for ${symbol}:`, error);
    }
    return [];
  }

  private async fetchNSEDirectData(symbol: string, timeframe: string): Promise<HistoricalPrice[]> {
    try {
      // Use NSE official website data (free but may have rate limits)
      const days = this.getTimeframeDays(timeframe);
      const endDate = new Date();
      const startDate = new Date();
      startDate.setDate(endDate.getDate() - days);
      
      const response = await fetch(`https://www.nseindia.com/api/historical/cm/equity?symbol=${symbol}&series=["EQ"]&from=${startDate.toISOString().split('T')[0]}&to=${endDate.toISOString().split('T')[0]}`, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`NSE Direct API failed: ${response.status}`);
      }
      ...
      if (data && data.data && Array.isArray(data.data)) {
        return data.data.map((item: any) => ({
          ...
          open: parseFloat(item.CH_OPENING_PRICE) || 0,
          high: parseFloat(item.CH_TRADE_HIGH_PRICE) || 0,
          low: parseFloat(item.CH_TRADE_LOW_PRICE) || 0,
          close: parseFloat(item.CH_CLOSING_PRICE) || 0,
          volume: parseInt(item.CH_TOT_TRADED_QTY) || 0
        }));
      }
    } catch (error) {
      console.log(`NSE Direct API failed for ${symbol}:`, error);
    }
    return [];
  }
  ...
  async getStockNews(symbol: string): Promise<NewsItem[]> {
    try {
      ...
      // If real news fails, return empty array (no mock data)
      console.log(`Real news APIs failed for ${symbol}. No mock news returned.`);
      return [];
    } catch (error) {
  ...
  private async fetchFromEconomicTimesRSS(symbol: string): Promise<NewsItem[]> {
    try {
      // Use multiple Economic Times RSS feeds for better coverage
      const feeds = [
        'https://economictimes.indiatimes.com/markets/rssfeeds/1977021501.cms',
        'https://economictimes.indiatimes.com/markets/stocks/rssfeeds/2146842.cms',
        'https://economictimes.indiatimes.com/news/company/corporate-trends/rssfeeds/13358350.cms'
      ];
      ...
      // Enhanced filtering for company-specific news
      const companyKeywords = this.getCompanyKeywords(symbol);
      return allItems.filter(item => 
        ...
        (item.headline.toLowerCase().includes('stock') && item.headline.toLowerCase().includes('market'))
      ).slice(0, 3);
  ...
  private getCompanyKeywords(symbol: string): string[] {
    // Map stock symbols to company names and related keywords for better news filtering
    const companyKeywords: Record<string, string[]> = {
      'RELIANCE': ['reliance', 'ril', 'mukesh ambani', 'jio', 'petrochemicals'],
      'TCS': ['tcs', 'tata consultancy', 'tata group', 'it services'],
      'INFY': ['infosys', 'murthy', 'it services', 'software'],
      'HDFCBANK': ['hdfc bank', 'hdfc', 'banking', 'private bank'],
      'ICICIBANK': ['icici bank', 'icici', 'banking', 'private bank'],
      'HINDUNILVR': ['hindustan unilever', 'hul', 'fmcg', 'consumer goods'],
      'ITC': ['itc limited', 'tobacco', 'cigarettes', 'fmcg'],
      'SBIN': ['sbi', 'state bank', 'public sector bank', 'banking'],
      'BHARTIARTL': ['bharti airtel', 'airtel', 'telecom', 'sunil mittal'],
      'LT': ['larsen toubro', 'l&t', 'engineering', 'construction'],
      'ADANIGREEN': ['adani green', 'adani', 'renewable energy', 'solar', 'gautam adani'],
      'ADANIGREENENERGY': ['adani green energy', 'adani', 'renewable', 'gautam adani']
    };
    
    return companyKeywords[symbol] || [symbol.toLowerCase()];
  }
