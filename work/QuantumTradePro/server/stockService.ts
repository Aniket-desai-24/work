...
export class StockService {
  ...
  private async fetchIndianHistoricalData(symbol: string, timeframe: string): Promise<HistoricalPrice[]> {
    try {
      // Try using NSE official data or Indian API service for historical data
      const response = await fetch(`${this.nseBseAPIUrl}/nse/stock/${symbol}/historical?period=${timeframe}`);
      
      if (!response.ok) {
        throw new Error(`Indian historical API failed: ${response.status}`);
      }
